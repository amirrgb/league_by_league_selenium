...
class methodss:
    global loadUrl;global workOption;global closeHelpUsImproveWindow;global acceptAllCookies;global findElement;global driver
    global Tab;global start;global scrollDown;global leaguesExtractor;global insertToLogFile;global isMatchForMenAndAdults;global insertLeagueLink
    ...
    def Tab(driver:webdriver,add:int=0):
        ...
        time.sleep(2)
        return driver
    def findElement(self,xxPath:str,level:str,finds:bool=False,refreshTime:int=16,Get_None:bool=False,timeout:int=20):
        n=0
        while True:
            n+=1
    ...
    def insertLeagueLink(country:str,league:str,link:str):
        ...
        cursor = cnx.cursor(buffered=True)
        sql=('UPDATE studied_league SET leagueLink = "%s" where leagueCountry = "%s" and leagueName = "%s";'%(link,country,league))
        cursor.execute(sql)
        cnx.commit()
        cnx.close()        
            ...
            if byRound is not None :
                ...
                while True:
                    if byRound.is_displayed():
                        ...
                        whichRound.click()
                        time.sleep(2)
                        firstRounds=driver.findElement(xFirstRound,"finding first Round in Rounds",finds=True,Get_None=True)
...
def goToLeague(driver,leagueLink):
    ...
    while not isEnd:
        ...
        driver.find_element(By.XPATH, '/html/body').send_keys(Keys.HOME)
        if currentYear.find("/")>=1:standardYear="20"+currentYear[:3]+"20"+currentYear[3:]
        else:standardYear=currentYear
